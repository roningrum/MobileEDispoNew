...
class AgendaInternal : AppCompatActivity() {
    lateinit var binding: ActivityAgendaInternalBinding
    ...
    lateinit var adapter: KegiatanInternalAdapter
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel.getBidang()
        viewModel.getKegiatanInternalBidang("1", GetDate.getTodayDate())
        ...
        binding.spinnerBidang.onItemSelectedListener = object : AdapterView.OnItemSelectedListener{
            override fun onItemSelected(
                ...
                if(binding.etTglCari.length()<1){
                    viewModel.getKegiatanInternalBidang(idBidang, GetDate.getTodayDate())
                    setJudulPencarianHariIni(binding.spinnerBidang.selectedItem.toString())
                }
                else{
                    viewModel.getKegiatanInternalBidang(idBidang, tglUbah!!)
    ...
    fun setJudulPencarianHariIni(bidang:String){
        binding.txtTglAgenda.text = "Agenda internal bidang $bidang hari ini ${GetDate.getTodayDate()}"
    }


    fun setJudulPencarianTanggal(bidang:String, tanggal:String){
        binding.txtTglAgenda.text = "Agenda internal bidang $bidang tanggal $tanggal"
    }
}